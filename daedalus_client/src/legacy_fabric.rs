...
pub async fn retrieve_data(
    ...
    let mut list = fetch_legacy_fabric_versions(None, semaphore.clone()).await?;
    let old_manifest = daedalus::modded::fetch_manifest(&format_url(&format!(
        "legacy-fabric/v{}/manifest.json",
        daedalus::modded::CURRENT_LEGACY_FABRIC_FORMAT_VERSION,
    )))
        .await
        .ok();
    ...
    const DUMMY_GAME_VERSION: &str = "1.8.9";
    ...
    let loader_versions = futures::future::try_join_all(
        loaders_mutex.read().await.clone().into_iter().map(
            |(stable, loader)| async {
                ...
                let version = fetch_legacy_fabric_version(
                    DUMMY_GAME_VERSION,
                    &loader,
                    semaphore.clone(),
                )
...
async fn fetch_legacy_fabric_version(
    version_number: &str,
    loader_version: &str,
    semaphore: Arc<Semaphore>,
) -> Result<PartialVersionInfo, Error> {
    fetch_fabric_like_version(version_number, loader_version, semaphore, LEGACY_FABRIC_META_URL).await
}

async fn fetch_legacy_fabric_versions(
    url: Option<&str>,
    semaphore: Arc<Semaphore>,
) -> Result<FabricVersions, Error> {
    fetch_fabric_like_versions(url, semaphore, LEGACY_FABRIC_META_URL).await
}